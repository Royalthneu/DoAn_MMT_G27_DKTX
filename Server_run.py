...
def main():
    ...
    if Connection.check_port_open(server_port):
        ...
        response = input(f"Cổng {server_port} chưa mở. Bạn có muốn mở cổng {
                         server_port} không? (y/n): ")
        if response.lower() == 'y':
    ...
    try:
        while True:
            ...
            if buffer.startswith("LIST_APPS_RUNNING"):
                ...
                Server.SV_app_process.start_app_by_path(
                    client_socket, app_path)
            ...
            # List / Start / Stop Services (Processes) đang chạy SERVER
                ...
                Server.SV_services_process.start_service(
                    client_socket, service_name)
            elif buffer.startswith("STOP_SERVICE"):
                service_name = buffer.split(" ", 1)[1]
                Server.SV_services_process.stop_service(
                    client_socket, service_name)
