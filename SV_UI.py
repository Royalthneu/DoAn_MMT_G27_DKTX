...
def create_label(window, text, relx, rely, width):
    return tk.Label(window, text=text, background="#d9d9d9", foreground="#000000",
             font="-family {Segoe UI} -size 9", anchor='w').place(
...
def create_button(window, text, relx, rely, command=None, state = "normal"):
    return tk.Button(window, text=text, background="#d9d9d9", foreground="#000000",
              font="-family {Segoe UI} -size 9", command=command, state=state).place(
...
lbl_status = tk.Label(window, text="Server is closing", background="#d9d9d9", foreground="#000000", 
                        font="-family {Segoe UI} -size 9", anchor='w')
lbl_status.place(relx=0.300, rely=0.58, height=20, width=300)

lbl_ip = tk.Label(window, text="Bí mật", background="#d9d9d9", foreground="#000000", 
                        font="-family {Segoe UI} -size 9", anchor='w')
...
def main():
    ...
    #     
    # else:
    #     print(f"Cổng {port} chưa mở. Đang mở cổng {port}...")
    ...
    if Connection.check_ip_address_valid(server_ip) and Connection.check_port_valid(port):
        ...
        messagebox.showinfo(title = 'THÔNG BÁO', message=F'Server is listenning on: {server_ip}:{port}')
        ...
        messagebox.showinfo(title = 'THÔNG BÁO', message='Địa chỉ IP hoặc Port không hợp lệ hoặc không mở.')        
